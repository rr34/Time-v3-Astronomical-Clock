...
# moments_parameters are tuple of 3('now' or single datetime moment, total duration seconds, step seconds)
def moments_generator(start, duration, step):
    if start == 'now':
        # start_moment = datetime.datetime.now(datetime.timezone.utc)
        start_moment = np.datetime64('now')
    else:
        start_moment = start

    end_moment = start_moment + np.timedelta64(duration, 's')
    step_length = np.timedelta64(step, 's')
    
    moments = np.arange(start_moment, end_moment, step_length)
...
def calculate_astro_risesandsets(moment_now, earth_latlng, elevation):
    ...
    return sun_daily, moon_daily, day_night_length_str
